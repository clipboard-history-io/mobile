...
const ClipboardItem: React.FC<{
  ...
  const isDeleteDisabled = React.useMemo(() => {
    ...
    return false;
  }, [settings.allowDeletingFavorites, settings.allowDeletingCloudItems, item.favorite, item.isInCloud]);
  ...
  return (
    <View key={item.id} className="w-full bg-background">
      <TouchableOpacity onPress={copyToClipboard} activeOpacity={0.7}>
        <CardHeader className="flex-row justify-between items-center">
          <CardTitle
            numberOfLines={1}
            ellipsizeMode="tail"
            className="text-base flex-1 pr-8"
          >
            {item.text}
          </CardTitle>
      ...
      <CardFooter className="flex-row justify-end gap-6">
        ...
        {item.tags.length > 1 ? <Text className="text-sm justify-end text-muted-foreground">+ {item.tags.length - 1}</Text> : null}</View> : null}
        <Text className="text-sm text-muted-foreground">
          {formatTimestamp(item.timestamp)}
        </Text>
        <TouchableOpacity disabled={user == 'Guest'} onPress={() => handleCloudPress(item.id)}>
          {user !== 'Guest' ? <Cloud size={18} className="text-blue-500" fill={item.isInCloud ? "#3b82f6" : "none"}/> : <CloudOff size={18} className="text-gray-400" />}
        </TouchableOpacity>
